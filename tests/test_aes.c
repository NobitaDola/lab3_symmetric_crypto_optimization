...
int main()
{
    ...
    uint8_t out_basic[16], out_ttable[16], out_ni[16], out_openssl[16], recovered[16];
    ...
    aes128_decrypt_basic(out_basic, recovered, basic_rk);
    ...
    if (memcmp(out_openssl, out_basic, 16) == 0 &&
        ...
        memcmp(out_openssl, out_ni, 16) == 0 && memcmp(recovered, in, 16) == 0)
    {
