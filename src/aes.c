...
void aes128_decrypt_basic(const uint8_t *in, uint8_t *out, const uint8_t *round_keys)
{
    ...
    for (unsigned i = 0; i < 16; ++i)
        state[i] = in[i] ^ round_keys[160 + i];
    for (int round = 9; round >= 0; --round) {
        /* Inverse ShiftRows followed by Inverse SubBytes. */
        temp[0]=inv_sbox[state[0]];   temp[4]=inv_sbox[state[4]];
        temp[8]=inv_sbox[state[8]];   temp[12]=inv_sbox[state[12]];
        temp[1]=inv_sbox[state[13]];  temp[5]=inv_sbox[state[1]];
        temp[9]=inv_sbox[state[5]];   temp[13]=inv_sbox[state[9]];
        temp[2]=inv_sbox[state[10]];  temp[6]=inv_sbox[state[14]];
        temp[10]=inv_sbox[state[2]];  temp[14]=inv_sbox[state[6]];
        temp[3]=inv_sbox[state[7]];   temp[7]=inv_sbox[state[11]];
        temp[11]=inv_sbox[state[15]]; temp[15]=inv_sbox[state[3]];
        for (unsigned i = 0; i < 16; ++i)
            temp[i] ^= round_keys[16 * round + i];
        if (round == 0) {
            memcpy(state, temp, 16);
        } else {
            for (unsigned c = 0; c < 4; ++c) {
                const uint8_t *a = temp + 4 * c;
                state[4*c]   = gf8_mul(a[0],14)^gf8_mul(a[1],11)^gf8_mul(a[2],13)^gf8_mul(a[3],9);
                state[4*c+1] = gf8_mul(a[0],9)^gf8_mul(a[1],14)^gf8_mul(a[2],11)^gf8_mul(a[3],13);
                state[4*c+2] = gf8_mul(a[0],13)^gf8_mul(a[1],9)^gf8_mul(a[2],14)^gf8_mul(a[3],11);
                state[4*c+3] = gf8_mul(a[0],11)^gf8_mul(a[1],13)^gf8_mul(a[2],9)^gf8_mul(a[3],14);
            }
        }
    }
    memcpy(out, state, 16);
...
void aes128_key_expansion_dec_ni(const __m128i *enc, __m128i *dec)
{
    dec[0] = enc[10];
...
void aes128_decrypt_ni(const uint8_t *in, uint8_t *out, const __m128i *rk)
{
    __m128i m = _mm_xor_si128(_mm_loadu_si128((const __m128i *)in), rk[0]);
    for (unsigned i = 1; i < 10; ++i)
        m = _mm_aesdec_si128(m, rk[i]);
    m = _mm_aesdeclast_si128(m, rk[10]);
    _mm_storeu_si128((__m128i *)out, m);
}
